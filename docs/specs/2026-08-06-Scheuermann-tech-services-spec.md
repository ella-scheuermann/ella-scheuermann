...
All calculations must use the named ranges defined in Section 2. No formulas may use Excel cell addresses. Each major calculation must appear as a separate, visible formula so that the workbook can be audited easily. Required outputs must be formula-driven and may not be entered as hardcoded values.
...
The sensitivity analysis will present the results in a comparison table and a single line chart.
...
2. **Formula Integrity**
   ...
   Every calculated output must be generated by a formula. No gray output cells may contain manually entered values.

3. **Named-Range Consistency**

   All workbook calculations must reference the named ranges defined in Section 2 rather than Excel cell addresses.
...
All summary outputs and tables will update automatically whenever an input value changes. Every output in the workbook will be formula-driven, update automatically whenever an input changes, and will never contain hardcoded values.
